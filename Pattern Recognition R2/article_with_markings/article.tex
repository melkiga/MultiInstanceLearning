...
In traditional classification, learning algorithms attempt to correctly label unknown samples by finding patterns between training samples and their class label. \textit{Multi-instance} (MI) learning is a generalization of \textit{supervised learning} that has been recently been gaining interest because of its applicability to many real-world problems such as webmining and text categorization {\citep{Andrews2002}}, image classification and retreival \citep{qi2007,cao2017weakly}, human action recognition \citep{Yi2016}, medical diagnosis and imaging {\citep{Herrera2016}}, and bioinformatics {\citep{Dietterich1997}} (drug activity prediction and protein identification). {\color{red}In the case of drug activity prediction, MIL allows molecules to be represented as related instances contained in a bag rather than single instances with their own labels.}
...
This implies that individual instance labels $y_i$ exist, but are not known (for positive bags) during training. Equation~\eqref{eqn:stas} can also be rewritten as $Y_I = \text{argmax}_{\forall i \in I}\,\,\, y_i$, for simplicity.

In addition to the SMI assumption, alternative MI assumptions have been proposed to date~\citep{Foulds2010}. A recent review describing the taxonomy of multi-instance classification was presented by \citet{Amores2013}. Classifiers that fall under the \textit{instance-space paradigm} aim to separate instances in positive bags from those in negative ones. Bag-level classifiers (\textit{bag-space paradigm}) treat each bag as a whole entity, implicitly extracting information from each bag in order to accurately predict their labels~\citep{qiao2017diversified}. Methods that fall under the \textit{embedded-space paradigm} map the input bags to a single feature vector that explicitly encapsulates all the relevant information contained within each bag.

Instance-based methods under the SMI assumption attempt to identify desirable instance properties that make a bag positive. One traditional method is the Axis-Parallel Rectangle (APR) \citep{Dietterich1997}, which assigns a positive label to an instance if it belongs to an APR. The Diverse Density (DD){~\citep{Maron1998}} measure is maximized for instances that are near at least one instance in a positive bag and far from all instances in negative bags.~\citet{Auer2004} present a boosting approach that uses balls centered around positive bags, called Multi-Instance Optimal Ball (MIOptimalBall). A major challenge of these methods is that the distributions of positive and negative bags affect their performance~\citep{Chen2006,Chen2004}. Alternatively, \citet{Fu2011} models the distribution of negative bags with Gaussian kernels, which can prove difficult when the quantity of data is limited.

Some methods in literature~\citep{Ray2005,Viola2005}, such as Simple-MI~\citep{Dong2006}, transform the MI dataset to a traditional instance-level dataset. This approach was evaluated by~\citet{Bunescu2007} and they proposed mapping each bag to a max-min vector. The disadvantage of these approaches is that they assume the labels of all instances within positive bags are positive, when this may not be the case.
\begin{figure*}
\centering
\small
...
An extension of traditional single-instance $k$-nearest neighbors method (KNN) was proposed by~\citet{Wang2000} to be applied to the bag-level, named CitationKNN. This method uses a distance function between bags in order to determine bag similarities and a voting scheme to determine the bag class labels.~\citet{Xu2011} proposed a multi-instance Decision-Based Neural Network (MI-DBNN), which is a neural network with a modular structure{~\citep{Herrera2016}}.

~\citet{Blockeel2005} introduced the Multi-Instance Tree Inducer (MITI), which uses decision trees as a heuristic to solve the MI problem. MITI aims to identify whether an instance within a bag is truly positive and eliminate false positives within the same bag. The disadvantage of this approach stems from removing instances considered as false positives from partially grown trees without updating the existing tree structure.~\citet{Bjerring2011} then enhanced this approach by creating the method Multi-Instance Rule Induction (MIRI), which eliminates any possibility of a suboptimal split because the tree is discarded and regrown. Other popular methods include MIBoost~\citep{Hall2009}, MIWrapper~\citep{Frank2003}, and Two-Level Classifier (TLC)~\citep{Wang2014}.

For most of the methods described above, implicit or explicit assumptions have been made about the distribution of the data~\citep{2017-KNOSYS-MIMV}. Selecting a method that is robust for a problem such as MIL can be difficult when little is known about the nature of the data, especially considering the unknown distribution of the instances within bags~\citep{2015-KAIS,2016-INS-MIARM}. {\color{red}Our prososed contribution,} MIRSVM, is an iterative method that uses SVMs to design a MIL model without making prior assumptions about the data. Classifiers of this type are known to provide better generalization capabilities, performance, and sparser models.

\subsection{Support Vector Machines}\label{subsec:svms}
Most classical learning techniques require knowledge of the data distribution to build accurate models, which is a serious restriction because, in most cases, the distribution is unknown~\citep{Kecman2001}. SVMs represent learning techniques that have been introduced under the \textit{structural risk minimization} (SRM) framework and VC theory. Rather than optimizing over L1 or L2 norms and classification error, SVMs perform SRM~\citep{Shalev2014}, minimizing the expected probability of classification error, resulting in a generalized model without making assumptions about the data distribution~\citep{Cortes1995}. 

SVMs are a particularly useful for learning linear predictors in high dimensional feature spaces, which is a computationally complex problem~\citep{wang2016hierarchical}. In the context of classification, this problem is approached by searching for the optimal \textit{maximal margin} of separability between classes~\citep{carrasco2015multi,fan2017mreklm}. Since most real-world datasets are non-linearly separable, ~\citet{Cortes1995} introduced the \textit{Soft-Margin L$1$-SVM}. However, if the training dataset is not linearly separable, the classifier may not have a good generalization capability~\citep{Nocedal2006}. Generalization and linear separability can be enhanced by mapping the original input space to a higher dimensional dot-product space by using a kernel function shown in Equation~\eqref{eqn:kerneltrick}:
\begin{equation}
...
where $C \in \reals$ is the penalty parameter that controls the trade-off between margin maximization and classification error minimization.
...
\citet{Andrews2002} proposed a mixed-integer quadratic program that solves the MI problem at an instance-level, using a support vector machine, named MISVM, that can be solved heuristically. Rather than maximizing the margin of separability between instances of different classes, this instance-based method maximizes the margin between bags. It tries to identify the key instance from each positive bag that makes the bag positive by assuming it has the highest margin value~\citep{Herrera2016}. Using bag-representatives from the positive class class and all instances from the other is an example of an approach that combines rules from the SMI assumption and the collective assumption.~\citet{Wang20143106} proposed a multi-criteria decision making system that measures the significance of unlabeled instances and selects the best instance iteratively using MISVM. A disadvantage of this approach stems from the assumption that all instances within positive bags are also positive, which is an implicit step in the initialization of MISVM.~\citet{Andrews2002} also proposed a second mixed-integer instance-level approach, named mi-SVM, which tries to identify the instances within positive bags that are negative and utilize them in the construction of the negative margin. The main disadvantage of these approaches is that they create an imbalanced class problem that favors the negative class, resulting in a biased classifier.
...
One of the first bag-level approaches to the multi-instance SVM problem was proposed by \citet{Smola2002}, who defined a bag-level kernel, which determines the similarity between two bags in a higher dimensional space.~\citet{Blaschko2006} propose conformal kernels which manipulate each attribute's dimension based on its importance. 
...
\end{alignat}
\end{subequations} 
where $\bm S_I$ is a set of the bag representatives' indices, $\bm x_{s_I}$ is the instance representative of bag $\mathcal{B}_I$, and the slack variable $\bm \xi \in \reals^m$ allows for optimizing over bag errors. Note the variables in MIRSVMs formulation are the similar to those of the traditional SVM, except they are now representing each bag as an instance. This is one of the benefits of the MIRSVM formulation, whereby using a bag-representative selector, traditional SVM solvers can be used for new data representations such the multi-instance data.

Solving the optimization problem given in Equation~\eqref{eq:primalmirsvm} using a quadratic programming solver is a computationally expensive task due to the number of constraints, which scales by the number of bags $n$, as well as the calculation of the inner product between two $d$-dimensional vectors in constraint~\eqref{eq:primalmirsvm1}. The proposed solution for these problems was deriving and solving the dual of the optimization problem given by Equation~\eqref{eq:primalmirsvm}. 

The dual can be formed by taking the Lagrangian of~{\eqref{eq:primalmirsvm}}, $\mathcal{L}(\bm{w},b,\bm{\xi},\bm{\alpha},\bm{\beta})$, and substituting the optimality conditions (from $\bigtriangledown_{w,b,\xi}\mathcal{L}(\bm{w},b,\bm{\xi},\bm{\alpha},\bm{\beta}) = 0$), where $\bm \alpha$ and $\bm \beta$ are the non-negative Lagrange multipliers. The dual MIRSVM formulation becomes the following:
\begin{subequations} 
...
We then kernelize the dual by replacing the inner product of the samples in feature space with their corresponding kernel values, $\spa{K}\left(\bm{x}_{s_I},\bm{x}_{s_K}\right)$. One of the biggest advantages of the dual SVM formulation is the sparseness of the resulting model, because support vectors, instances that have their corresponding $\alpha_I \neq 0$, are only considered when forming the decision boundary. MIRSVM uses a Gaussian RBF kernel, given by Equation~\eqref{eq:gaussiankernel}, where $\sigma$ is the Gaussian shape parameter.
\begin{equation}
...
\end{tikzpicture}}
\caption{This figure represents a summary of the steps performed by MIRSVM. The representatives are first randomly initialized, then continuously updated according to the current hyper-plane, which is found using a quadratic programming (QP) solver. Upon completion, the model is returned along with the optimal bag-representatives.}
\end{minipage}
...
SimpleMI achieves the highest rank and competitive run times because, {\color{red}rather than using the instances} in each bag to train a model, it takes the mean value of the instances in a bag and uses that for training. Even though SimpleMI has fast run-times, its performance over the previous metrics has been shown to be random and not as effective as the bag-level methods.
...
The experimental study showed the better performance of MIRSVM compared with state-of-the-art multi-instance support vector machines, traditional multi-instance learners, as well as ensemble methods. The results, according to a variety of performance metrics, were compared and further validated using statistical analysis with non-parametric tests which highlight the advantages of using bag-level based and ensemble learners, such as miGraph, Bagging, and Stacking, while showing the instance-level based learners performed poorly in comparison or were deemed as strongly biased and unstable classifiers. {\color{red}This was especially reflected in the meta-ranks comparison, shown in Table~\ref{tab:metarank}, over Accuracy, Precision, Recall, Cohen's Kappa, AUC, and run time, where instance-level learners were ranked last, whereas bag-level and ensemble learners were better in rank, and MIRSVM ranked first.} Our proposal, MIRSVM, performs statistically better, neither compromising accuracy nor run-time while displaying a robust performance across all of the evaluated datasets.

{\color{red}MIRSVM could be further improved by invesitgating the following approaches. Rather than using a QP solver at each iteration of MIRSVM, a different optimal solver could be experimented with. Due to the run time implications of using a QP solver, using a different solver might improve MIRSVM's scalability. Another area to investigate would be implementing MIRSVM in high-performance distributed environments. A third concept to explore would be testing MIRSVM on highly imbalanced multi-instance datasets and observe its performance. MIRSVM could also be extended to handle multi-class multi-instance problems by using traditional methods such as one-versus-one, where $c(c - 1)/2$ classifiers are trained for a $c$-class problem, and one-versus-all, where a single classifier is trained per class~\citep{bishop2007pattern}. }

\section*{Acknowledgment}
This research was supported by the Spanish Ministry of Economy and Competitiveness, project TIN2017-83445-P and by FEDER funds.
