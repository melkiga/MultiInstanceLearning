...
\newtcolorbox{highlighted}{colback=lightyellow,coltext=black,breakable,boxrule=0pt,frame hidden}
...
This implies that individual instance labels $y_i$ exist, but are not known (for positive bags) during training. Equation~\eqref{eqn:stas} \hl{can also be rewritten as $Y_I = \text{argmax}_{\forall i \in I}\,\,\, y_i$, for simplicity.}

In addition to the SMI assumption, alternative MI assumptions have been proposed to date~\citep{Foulds2010}. \hl{A recent review describing the taxonomy of multi-instance classification was presented by }\citet{Amores2013}. \hl{Classifiers that fall under the \textit{instance-space paradigm} aim to separate instances in positive bags from those in negative ones.} Bag-level classifiers (\textit{bag-space paradigm}) treat each bag as a whole entity, implicitly extracting information from each bag in order to accurately predict their labels~\citep{qiao2017diversified}. Methods that fall under the \textit{embedded-space paradigm} map the input bags to a single feature vector that explicitly encapsulates all the relevant information contained within each bag.

\hl{Instance-based methods under the SMI assumption attempt to identify desirable instance properties that make a bag positive. One traditional method is the Axis-Parallel Rectangle (APR) {\citep{Dietterich1997}}, which assigns a positive label to an instance if it belongs to an APR. The Diverse Density (DD){~\citep{Maron1998}} measure is maximized for instances that are near at least one instance in a positive bag and far from all instances in negative bags.}~\citet{Auer2004} \hl{present a boosting approach that uses balls centered around positive bags, called Multi-Instance Optimal Ball (MIOptimalBall). A major challenge of these methods is that the distributions of positive and negative bags affect their performance}~\citep{Chen2006,Chen2004}. \hl{Alternatively, }\citet{Fu2011} \hl{models the distribution of negative bags with Gaussian kernels, which can prove difficult when the quantity of data is limited.}

Some methods in literature~\citep{Ray2005,Viola2005}, such as Simple-MI~\citep{Dong2006}, \hl{transform the MI dataset to a traditional instance-level dataset. This approach was evaluated by}~\citet{Bunescu2007} \hl{and they proposed mapping each bag to a max-min vector. The disadvantage of these approaches is that they assume the labels of all instances within positive bags are positive, when this may not be the case.}
\begin{figure*}
\centering
\small
...
\hl{where $C \in \reals$ is the penalty parameter that controls the trade-off between margin maximization and classification error minimization.}
...
One of the first bag-level approaches to the multi-instance SVM problem was proposed by \citet{Smola2002}, \hl{who defined a bag-level kernel, which determines the similarity between two bags in a higher dimensional space.}~\citet{Blaschko2006} propose conformal kernels which manipulate each attribute's dimension based on its importance. 
...
\end{alignat}
\end{subequations} 
\hl{where $\bm S_I$ is a set of the bag representatives' indices, $\bm x_{s_I}$ is the instance representative of bag $\mathcal{B}_I$, and the slack variable $\bm \xi \in \reals^m$ allows for optimizing over bag errors. Note the variables in MIRSVMs formulation are the similar to those of the traditional SVM, except they are now representing each bag as an instance. This is one of the benefits of the MIRSVM formulation, whereby using a bag-representative selector, traditional SVM solvers can be used for new data representations such the multi-instance data.}
...
\end{tikzpicture}}
\caption{\hl{This figure represents a summary of the steps performed by MIRSVM. The representatives are first randomly initialized, then continuously updated according to the current hyper-plane, which is found using a quadratic programming (QP) solver.} Upon completion, the model is returned along with the optimal bag-representatives.}
\end{minipage}
...
\State $\bm G \leftarrow (\bm Y_S \times \bm Y_S) \bm \cdot \mathcal{K}\left( \bm X_S,\,\bm X_S,\,\sigma\right)$ \Comment{Build \hl{Gram} matrix}
