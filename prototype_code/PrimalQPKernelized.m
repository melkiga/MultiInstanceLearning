...
while(sum(S_old == S) ~= endlength)
    ...
    Xn = X(index,:);
    Yn = Y(index);
    G = grbf_fast(Xn,Xn,param);
    
    H = diag([ones(1,nnew), zeros(1, nnew+1)]);
    f = [zeros(1, nnew), C * ones(1, nnew), 0]';
    Aineq = [-diag(Yn)*G, -eye(nnew), -Yn];
    bineq = -ones(nnew, 1);
    LB = [-inf(1,nnew), zeros(1,nnew), -inf];

    z = quadprog(H,f,Aineq,bineq,[],[],LB);
    w = z(1:nnew,1);
    bias = z(end,1);
    ksi = z(nnew+1:end-1,1);
